...
def paired_random_crop(img_gts, img_lqs, lq_patch_size, scale, gt_path):
    ...
    # print(img_lqs[0].shape, img_gts[0].shape)
...
def paired_random_crop_tensor(img_gts, img_lqs, lq_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    # print(img_lqs[0].shape, img_gts[0].shape)
    gt_patch_size = [int(lq_patch_size[0] * scale), int(lq_patch_size[1] * scale)]
    ...
    # crop lq patch
    img_lqs = [
        v[..., top:top + lq_patch_size[0], left:left + lq_patch_size[1]]
        for v in img_lqs
    ]
    ...
    top_gt, left_gt = int(top * scale), int(left * scale)
    img_gts = [
        v[..., top_gt:top_gt + gt_patch_size[0], left_gt:left_gt + gt_patch_size[1]]
        for v in img_gts
    ]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_lqs
def paired_random_cropUHD(img_gts, img_lqs, lq_patch_size, scale, gt_path):
    """Paired random crop.
...
def paired_center_cropUHD(img_gts, img_lqs, lq_patch_size, scale, gt_path):
    """Paired random crop.
...
def random_augmentation(*args):
    ...
    return out
def data_augmentationUHD(image, mode):
    """
    Performs data augmentation of the input image
    Input:
        image: a cv2 (OpenCV) image
    ...
    return out
def random_augmentationUHD(*args):
    out = []
    flag_aug = random.randint(0,3)
    for data in args:
        out.append(data_augmentationUHD(data, flag_aug).copy())
    return out

# def random_augmentation(*args):
#     out = []
#     flag_aug = random.choice([0,1,4,5])
#     for data in args:
#         out.append(data_augmentation(data, flag_aug).copy())
#     return out
