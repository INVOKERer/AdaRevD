...
import random
import os
class Dataset_Multi_Paired_RGB(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = single_rgb_paths_from_folder([self.lq_folder], ['lq'], self.filename_tmpl)
    ...
    def __getitem__(self, index):
        ...
        if len(file_name_) > 2:
            ...
            end_ = -2 if self.opt['phase'] == 'train' else -1
            for k in file_name_[1:end_]:
                file_name_x = file_name_x + '_' + k
        else:
            file_name_x = file_name_[0] + '_0'
        # print(file_name_)
        # if self.opt['phase'] == 'train' and len(file_name_) > 2:
        #     gt_path = os.path.join(self.gt_folder, file_name_x + '_' + file_name_[-1])
        # elif self.opt['phase'] == 'train' and len(file_name_) == 2:
        #     gt_path = os.path.join(self.gt_folder, file_name_x + '.hdr')
        # else:
        gt_path = os.path.join(self.gt_folder, file_name_x + ext)
