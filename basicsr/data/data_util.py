...
def sort_key(x):
    return x['psnr']
def paired_paths_from_folder_sorted_by_psnr(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(gt_paths)):
        ...
        mse = np.mean((img_gt - img_input) ** 2)
...
def mpaired_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(gt_paths)):
        ...
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
def paired_hsi_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    input_paths = list(scandir(input_folder, suffix='hdr'))
    gt_paths = list(scandir(gt_folder, suffix='hdr'))
    assert len(input_paths) == len(gt_paths), (
        f'{input_key} and {gt_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}.')
    paths = []
    for idx in range(len(gt_paths)):
        ...
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
def single_hsi_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
