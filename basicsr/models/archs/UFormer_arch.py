...
from basicsr.models.archs.win_util import window_partitionx, window_reversex
...
class LeFF(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0., use_eca=False):
        super().__init__()
        self.linear1 = nn.Sequential(nn.Conv2d(dim, hidden_dim, kernel_size=1),
                                     act_layer())
        self.dwconv = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, groups=hidden_dim, kernel_size=3, stride=1, padding=1),
            act_layer())
        self.linear2 = nn.Sequential(nn.Conv2d(hidden_dim, dim, kernel_size=1))
        self.dim = dim
        self.hidden_dim = hidden_dim
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=LayerNorm, token_projection='linear', token_mlp='leff',
    ...
    def flops(self):
        ...
        return flops
class Attention_intrak_uformer(nn.Module):
    def __init__(self, dim, num_heads, bias, window_size=32, k=8, shift_size=0, relu=False, irelu=False,
                 shift_method='roll'):
        super().__init__()
        self.num_heads = num_heads
        self.shift_size = shift_size
        ...
        self.k = k
        if self.k > 1:
            self.attn = Attn_Block_UFormer(dim, (window_size, window_size), num_heads, win_size=self.win_size[0], shift_size=0, modulator=True)
        else:
            self.attn = Attn_Block_UFormer(dim, (window_size, window_size), num_heads, win_size=self.win_size[0], shift_size=shift_size,
                                           modulator=True)
        # self.window_size = window_size
    ...
    def forward(self, x):
        ...
        if self.k > 1:
            ...
            x = rearrange(x, 'b c (k1 h) (k2 w) -> b c (h k1) (w k2)', k1=self.k, k2=self.k)
            x = self.attn(x)
            x = rearrange(x, 'b c (h k1) (w k2) -> b c (k1 h) (k2 w)', k1=self.k, k2=self.k)
            if self.window_size is not None and (H != self.window_size or W != self.window_size):
                # if self.shift_size == 0:
    ...
    def flops(self, H, W):
        ...
        return flops
class LeWinTransformerBlock2(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=LayerNorm, token_projection='linear', token_mlp='leff',
                 modulator=False, cross_modulator=False):
        ...
        self.norm1 = norm_layer(dim)
        self.attn = Attention_intrak_uformer(dim, num_heads, bias=True, window_size=input_resolution[0],
                                                   k=input_resolution[0]//self.win_size, shift_size=shift_size)
    ...
    def flops(self):
        ...
        # print("LeWin:{%.2f}"%(flops/1e9))
        return flops
class Attn_Block_UFormer(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 norm_layer=LayerNorm, token_projection='linear', token_mlp='leff',
                 modulator=False, cross_modulator=False):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        # build blocks
        LeWinTransformerBlock = LeWinTransformerBlock2
        if shift_flag:
...
class Uformer(nn.Module):
    def __init__(self, img_size=64, in_chans=3, dd_in=3,
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer().cuda()
