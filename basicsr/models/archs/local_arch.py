...
class LocalInstanceNorm2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1,
                 affine=False, track_running_stats=False, train_size=None):
        super().__init__()
        ...
        self.avgpool = AvgPool2d(train_size=train_size)
        self.eps = eps
...
def replace_layers(model, base_size, train_size, fast_imp, **kwargs):
    for n, m in model.named_children():
        ...
        if isinstance(m, nn.InstanceNorm2d):
            norm = LocalInstanceNorm2d(num_features=m.num_features, eps=m.eps, momentum=m.momentum, affine=m.affine,
                                       track_running_stats=m.track_running_stats, train_size=train_size)
