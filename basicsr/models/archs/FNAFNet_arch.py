...
class FNAFBlock(nn.Module):
    def __init__(self, c, num_heads=1, window_size=0, window_size_fft=0, shift_size=-1, DW_Expand=2, FFN_Expand=2,
...
class FNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1],
                 window_size_e=[0,0,0,0], window_size_m=[0], window_size_e_fft=[0,0,0,0], window_size_m_fft=[0],
                 window_sizex_e=[0,0,0,0], window_sizex_m=[0], num_heads_e=[1, 2, 4, 8], num_heads_m=[16]):
        super().__init__()
        # num_heads_e = [1, 2, 4, 8]
        # num_heads_m = [16]
