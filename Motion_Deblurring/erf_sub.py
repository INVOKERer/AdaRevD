...
if __name__ == '__main__':
    ...
    out_dir = '/home/ubuntu/106-48t/personal_data/mxt/exp_results/ICCV2023/figs/ERF_sub1111'
    os.makedirs(out_dir, exist_ok=True)
    ERF_dict = {}
    attn_type_list = ['nodct_global_channel', 'channel_mlp', 'channel']
    target = attn_type_list[0]
    source = attn_type_list[1:]
    for attn_type in attn_type_list:
    # attn_type = 'channel' # 'nodct_global_channel'
        ...
        sns_plot = plt.figure()
        sns.heatmap(erf, cmap='Blues_r', linewidths=0.0, vmax=0.2, vmin=0.,
        ...
        out_way = os.path.join(out_dir, attn_type + '_heatmap' + '.png')
        sns_plot.savefig(out_way, dpi=700)
        ERF_dict[attn_type] = erf
    erf_t = ERF_dict[target]
    num_max = 0
    for s in source:
        erf_sub = ERF_dict[s] - erf_t
        num_max = max(num_max, np.abs(erf_sub).max())

    for s in source:
        erf_sub = ERF_dict[s] - erf_t
        sns_plot = plt.figure()
        sns.heatmap(erf_sub, cmap='RdBu_r', linewidths=0.0, vmax=num_max, vmin=-num_max,
                    xticklabels=False, yticklabels=False, cbar=True)  # RdBu_r Reds_r .invert_yaxis()
        # out_way = os.path.join(out_root, 'attn_matrix_cosine_tar-center_DCT_LN_local' + '.png')
        out_way = os.path.join(out_dir, s+'-' + target + '_heatmap_sub' + '.png')
        sns_plot.savefig(out_way, dpi=700)
